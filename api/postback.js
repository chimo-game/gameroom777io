...
module.exports = async function handler(req, res) {
  ...
  if (!offer_id && !lead_id) {
    return res
      .status(400)
      .json({ error: "Missing required parameters (offer_id or lead_id)" });
  }
  ...
  try {
    ...
    if (!writeRes.ok) {
      ...
      return res
        .status(200)
        .json({ status: "error_logged", detail: "Firestore write failed" });
    }
    ...
    if (statsRes.ok) {
      ...
      const currentCount = existing.fields?.conversions?.integerValue
        ? parseInt(existing.fields.conversions.integerValue)
        : 0;
      const currentRevenue = existing.fields?.revenue?.doubleValue
        ? parseFloat(existing.fields.revenue.doubleValue)
        : 0;
      const currentChargebacks = existing.fields?.chargebacks?.integerValue
        ? parseInt(existing.fields.chargebacks.integerValue)
        : 0;
      ...
      const updateDoc = {
        fields: {
          ...existing.fields,
          conversions: {
            integerValue: String(currentCount + (isChargeback ? 0 : 1)),
          },
          revenue: {
            doubleValue:
              currentRevenue + (isChargeback ? 0 : parseFloat(payout) || 0),
          },
          chargebacks: {
            integerValue: String(currentChargebacks + (isChargeback ? 1 : 0)),
          },
      ...
      await fetch(
        `${FIRESTORE_BASE}/vs7_daily_stats?documentId=${statsDocId}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
    ...
    console.log(
      `[Postback] ${isChargeback ? "Chargeback" : "Conversion"}: offer=${offer_id} payout=$${payout} lead=${lead_id} country=${country_code}`,
    );
    ...
    return res
      .status(200)
      .json({ status: "error_logged", detail: err.message });
  }
};
